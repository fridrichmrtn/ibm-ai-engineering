"""Gradio app to polish resume content using OpenAI chat completions."""
...
LOGGER = logging.getLogger("resume_polish_app")
...
def polish_resume(
    ...
    """Polish resume content for a target role using a selected model."""
    system_prompt = os.getenv(
        "SYSTEM_PROMPT",
        (
            "You are an expert resume editor. Improve clarity, impact, and relevance to "
            "the target role. Use action verbs, quantify results, and align content with "
            "job expectations. Do not include or request sensitive PII (e.g., full "
            "address, SSN, birthdate). Return a polished resume section in clear, "
            "professional language."
        ),
    )

    if polish_instruction and polish_instruction.strip():
        user_prompt = (
            "Given the resume content:\n\"\"\"\n"
            f"{resume_content}\n"
            "\"\"\"\n"
            f"Polish it for the {position_name} position based on these instructions:\n"
            f"{polish_instruction}"
        )
    else:
        user_prompt = (
            "Suggest improvements for the following resume content to better align with "
            f"a {position_name} role. Return the polished version and highlight key "
            "adjustments for clarity, relevance, and impact.\n"
            f"\"\"\"\n{resume_content}\n\"\"\""
        )
...
def _build_interface() -> gr.Interface:
    ...
    description = (
        "This application helps you polish your resume. Enter the position you want to "
        "apply for, your resume content, and optional instructions, then get a polished "
        "version.\n\n"
        "Note: Avoid including sensitive PII (e.g., full address, SSN, birthdate)."
    )
    ...
    iface = gr.Interface(
        ...
        inputs=[
            ...
            gr.Textbox(
                ...
                placeholder=(
                    "Enter specific instructions or areas for improvement (optional)..."
                ),
                lines=2,
            ),
            gr.Dropdown(
        ...
        examples=[
            [
                "Data Scientist",
                "Experienced analyst with Python and SQL experience. Worked on dashboards.",
                "Emphasize machine learning projects and quantified outcomes.",
                default_model,
            ],
            [
                "Backend Engineer",
                "Developed APIs in Flask. Worked with databases.",
                "Focus on scalability, performance, and reliability achievements.",
                default_model,
            ],
        ],
    )
